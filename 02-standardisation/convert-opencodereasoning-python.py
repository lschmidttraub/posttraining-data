...
def process_split(ds: Dataset, num_proc: int) -> Dataset:
    ...
    result = result.map(convert_row,
                    num_proc=num_proc,
                    desc="Converting opencodereasoning-python")
...
def save_dataset_and_metadata(dataset_dict: DatasetDict, output_path: Path, 
    ...
    processing_entry = {
        "operation": "convert_opencodereasoning-python",
        "script": "convert_opencodereasoning-python.py",
    ...
    if "source_dataset" not in metadata:
        metadata["source_dataset"] = "opencodereasoning-python"
...
def main():
    ...
    ds = load_dataset(str(inp), split="python")
